"""
Network analysis using NetworkX directly.

All functions work with standard NetworkX graphs (nx.DiGraph).
For NetworkXGraph wrappers, pass G.graph or G.to_networkx().

Functions
---------
runstats : Comprehensive network statistics with visualizations

Examples
--------
>>> from pyplenet_nx import NetworkXGraph
>>> from pyplenet_nx.analysis import runstats
>>> G = NetworkXGraph("my_network")
>>> runstats(G.graph)           # Option 1: Direct access
>>> runstats(G.to_networkx())   # Option 2: Via method
=== Network Statistics ===
...
def runstats(G, show_plots=True):
    """
    Comprehensive network statistics using NetworkX directly.

    Parameters
    ----------
    G : nx.DiGraph
        NetworkX directed graph to analyze
    show_plots : bool, optional
        Whether to show matplotlib plots (default True)

    Examples
    --------
    >>> from pyplenet_nx import NetworkXGraph
    >>> from pyplenet_nx.analysis import runstats
    >>> wrapper = NetworkXGraph("my_network")
    >>> runstats(wrapper.graph)           # Option 1
    >>> runstats(wrapper.to_networkx())   # Option 2 (same result)
    """
    ...
    print(f"Transitivity: {avg_transitivity:.4f}")
    ...
    in_degrees = sorted((d for n, d in G.in_degree()), reverse=True)
    out_degrees = sorted((d for n, d in G.out_degree()), reverse=True)
    ...
    if (out_degrees or in_degrees) and show_plots:
...
if __name__ == "__main__":
    """Example usage of network statistics."""
    from pyplenet_nx.core.graph import NetworkXGraph

    try:
        G = NetworkXGraph('graph_data')
        if G.graph.number_of_nodes() > 0:
            print("Loaded NetworkX graph")
            runstats(G.graph, show_plots=True)  # Pass G.graph to use NetworkX directly
        else:
            print("No graph data found in 'graph_data'")
            print("Generate a network first using generate.py")
